...
3.  **Glue Data Quality:** Billed per DPU-hour for executing data quality recommendations and evaluation rules.
4.  **Glue Data Catalog:** Billed for metadata storage (metadata objects) and catalog access requests.
...
*   **Glue Flex Execution (Non-Urgent Batches):** **$0.29 per DPU-hour** (a **34% direct discount**).
    *   *Usage:* Best for non-time-critical nightly ETL workloads. Flex jobs run on spare AWS capacity and can be reclaimed (like Spot Instances).
...
### B. AWS Glue for Ray (Deprecation Notice)
*   AWS Glue for Ray will be closed to **new customers starting April 30, 2026**. Existing workloads remain supported. AWS recommends transitioning to **Amazon EKS** using the open-source KubeRay Operator for managing Ray clusters.

### C. AWS Glue Data Quality
*   **Rules Evaluation:** Data Quality tasks (rules evaluations and recommendations) are billed at standard Glue compute rates: **$0.44 per DPU-hour** (or **$0.29 per DPU-hour** under Flex).
*   **Anomaly Detection:** Monitoring metrics for anomalies incurs an additional premium surcharge of **1 DPU per statistic** monitored for the duration of the detection job.
...
| **Data Quality DPU** | Per DPU-hour | **$0.4400** | 1 minute |
| **Glue Crawler DPU** | Per DPU-hour | **$0.4400** | **10 minutes** |
| **Catalog Storage** | Per 100K objects | $1.0000 | Over 1 Million free |

### Example Monthly Cost Calculation (Standard vs Flex)
*Workload: A nightly batch ETL job runs on AWS Glue in N. Virginia. The job requires 10 DPUs and runs for exactly 2 hours each night (60 hours/month).*

*   **Option A: Standard Spark Execution:**
    $$\text{Standard Cost} = 10\text{ DPUs} \times 60\text{ hours} \times \$0.44 = \$264.00\text{ / month}$$
*   **Option B: Flex Execution (34% Discount):**
    $$\text{Flex Cost} = 10\text{ DPUs} \times 60\text{ hours} \times \$0.29 = \$174.00\text{ / month}$$
*   **Net Savings:** **$90.00/month** (Flex execution directly cuts the ETL compute cost).
...
*   **Over-Provisioning DPUs on Simple Jobs:** Running standard Spark ETL jobs with high default DPU settings for small datasets.
...
*   **Development Interactive Sessions Left Open:** Keeping Glue notebook interactive sessions open and idle. Sessions default to a minimum of 5 DPUs (**$2.20/hour**).

---

## 7. Actionable Cost Optimization Strategies
1.  **Enable Flex Execution for Non-Urgent Jobs:**
    *   For any overnight data processing or reporting jobs that do not have strict completion deadlines, enable **Flex Execution**.
    *   **The Savings:** Cuts DPU costs by **34%**.
2.  **Optimize DPU Sizing (Scale Down Dev/Small Jobs):**
    *   For testing and development jobs, configure the **Maximum DPUs** parameter to the minimum limit (e.g., 2 DPUs). For simple, single-file processing tasks, write them as **Python Shell Jobs** (using 0.0625 DPU at $0.0275/hr) instead of full Apache Spark jobs.
3.  **Optimize Glue Crawler Runs (Avoid Crawler Storms):**
    *   Do not run crawlers on a schedule if S3 schemas do not change. Add partitions manually using the `ALTER TABLE ADD PARTITION` statement in Athena or the Glue catalog API.
