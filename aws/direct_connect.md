...
*   *Break-Even Math:* If your enterprise downloads 50 TB of data per month from AWS:
    *   *Via Internet:* $0.09 × 50,000 = **$4,500.00/month**.
    *   *Via 1 Gbps Direct Connect:* $0.30 × 730 hrs ($219) + $0.02 × 50,000 ($1,000) = **$1,219.00/month** (Save 73%!).
...
*   **Leaving Ordered Connections Active but Unconnected:** Ordering a Direct Connect port, receiving the Letter of Authorization (LOA), and delaying the datacenter cross-connect setup. AWS bills you for the port starting **90 days after ordering** (or when the port goes active), even if no fiber cable is physically plugged in.
...
1.  **Perform Break-Even Analysis (Hosted vs. Dedicated):**
    *   If your monthly data egress is under **10 TB**, use a **Hosted Connection** (e.g., 200 Mbps or 500 Mbps) through an AWS partner. This avoids the high base rate of dedicated physical ports.
    *   If monthly egress exceeds **20 TB**, deploy a **1 Gbps Dedicated Connection** to maximize the $0.02/GB egress discount.
2.  **Cancel Unconnected Direct Connect Ports:** If your datacenter migration is delayed, cancel the Direct Connect port request before the 90-day automatic billing window triggers, and re-order once physical fiber cabling schedules are locked.
3.  **Disable SiteLink Outside Active Transfers:** If you use SiteLink to replicate large backups between two offices once a week, write a script to disable the virtual interface or disable SiteLink configuration during idle weekdays to save the $0.50/hour connection charge.
4.  **Use AWS Transit Gateway with Direct Connect Gateway:** If connecting Direct Connect to multiple VPCs:
    *   Connect your Direct Connect link to a **Direct Connect Gateway (DXGW)** and link it to Transit Gateway.
    *   This consolidation allows up to 10 VPCs to share a single Direct Connect virtual interface, avoiding paying for multiple port hours.
