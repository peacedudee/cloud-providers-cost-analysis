...
## 2. Billing Mechanics
*   **Orchestration Cost:** There is **no additional charge** for using AWS Elastic Beanstalk.
...
*   **Load-Balanced Environment (Production Standard):** Beanstalk provisions an Auto Scaling Group, a Load Balancer (ALB), and at least two EC2 instances across AZs.
    *   *Baseline Cost:* Flat monthly ALB charge (~$16.42/month) plus vCPU/Memory for at least two EC2 instances and EBS volumes.
...
*   Beanstalk allows you to create a database instance (RDS) directly inside the environment.
...
|-------------------|---------------|------------------|--------------------------|
| **Beanstalk Fee** | Management | **$0.00** (Free) | N/A |
| **EC2 Instances** | Per running instance second | *See EC2 rates* | Use smaller instance types (e.g., t3/t4g) |
...
1.  **Configure Single-Instance Mode for Non-Prod:** In Dev, Test, and Staging environments, set the Environment Type to **Single-Instance**. This eliminates the hourly Load Balancer fee and halves your baseline compute cost.
2.  **Configure Beanstalk Application Version Lifecycle Rules:** In the Beanstalk console, set a lifecycle limit on application versions. Configure it to keep only the last 50 versions or delete versions when the total storage exceeds 5 GB. This automatically purges old ZIP bundles from S3.
3.  **Decouple RDS Databases:** Never create RDS instances directly inside your Beanstalk environment config. Provision your database separately via the RDS console, and pass the connection endpoint to Beanstalk via environment properties. This allows you to scale, stop, or manage the database independently.
4.  **Adopt Graviton (ARM) EC2 Instances:** Update the EC2 instance configuration in your Beanstalk platform settings to use Graviton instance families (e.g., `t4g.micro` or `t4g.medium`). This offers a **20% savings** on compute hours.
5.  **Utilize Auto Scaling Schedules:** If you run load-balanced environments for QA or internal testing, configure scheduled scaling actions in the Auto Scaling group. Set it to scale down to 0 instances at 6:00 PM and scale back up to 1 or 2 instances at 8:00 AM on weekdays.
