...
*   **Paying for AWS Directory Service When IAM Identity Center Suffices:**
    *   Deploying AWS Managed Microsoft AD ($175.20 to $584.00/month) strictly to provide corporate users with SSO logins to the AWS Management Console.
    *   *Alternative:* IAM Identity Center provides single sign-on out of the box for $0.00.
*   **Third-Party IdP User Over-Licensing:** Purchasing extra user seats on third-party identity providers (like Okta or Azure AD P2) for users who only need access to AWS accounts, when they could be managed directly within IAM Identity Center's free native directory.

---

## 7. Actionable Cost Optimization Strategies
1.  **Replace AWS Managed AD for Portal SSO:**
    *   If your organization launched AWS Managed Microsoft AD solely to authenticate developers logging into the AWS Console or CLI, migrate them to **IAM Identity Center**.
    *   Connect IAM Identity Center to your existing free/standard corporate IdP (such as Google Workspace or Microsoft Entra ID Free) via SAML 2.0.
...
2.  **Centralize CLI Credential Management (`aws sso login`):**
    ...
    *   *Why:* Eliminates manual access key rotation workflows, improves security posture, and enforces short-lived 1-hour to 12-hour temporary credentials for $0.00.
