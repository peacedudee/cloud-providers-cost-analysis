...
## 1. Service Overview
AWS Elemental MediaLive is a broadcast-grade live video processing service. It ingests live video streams (via RTP, RTMP, HLS, or MediaConnect) and encodes them in real-time into high-quality adaptive bitrate (ABR) streams for broadcast television and internet-connected devices. MediaLive supports advanced video codecs (AVC/H.264, HEVC/H.265, AV1) and multi-channel audio. MediaLive offers **Standard Pipelines** (dual-pipeline redundant high availability across Availability Zones) and **Single Pipelines**. It is billed per channel-hour based on input/output resolutions, frame rates, and codec configurations.

---

## 2. Billing Mechanics
1. **Running Channel Hours:** Billed per hour (or fraction of an hour, measured per second with a 1-minute minimum) that a MediaLive channel is in the `RUNNING` state.
...
4. **Reserved Pricing:** 1-Year commitments offer up to **75% savings** for continuous 24/7 linear channels.
