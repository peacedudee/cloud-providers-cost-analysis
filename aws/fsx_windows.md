...
6.  **Backup Storage:** Charged per GB-month for data backed up to S3.
...
*   **Single-AZ HDD:** **$0.013 per GB-month** (90% storage savings compared to SSD). Designed for enterprise home directories and departmental shares with low access frequencies.
...
*   *Example:* Provisioning 128 MB/s on a Multi-AZ file system costs:
    $$128\text{ MB/s} \times \$4.50 = \$576.00\text{ / month flat}$$
    This charge applies even if the file system is completely idle.
...
*   **Idle Throughput Oversizing:** Provisioning high throughput capacities (e.g., 256 MB/s or 512 MB/s) to handle rare, monthly batch processes, resulting in high monthly idle fees ($1,152/month for 256 MB/s Multi-AZ).
*   **Unnecessary Multi-AZ in Dev/QA:** Deploying Multi-AZ file systems for development, staging, or testing environments. High availability is rarely required in these environments, and switching to Single-AZ instantly cuts costs by **40–50%**.
*   **Storing Cold Shares on SSD:** Deploying SSD storage for standard user home directories or archive shares. HDD storage is **10x cheaper** than SSD ($0.013/GB vs. $0.130/GB) and perfectly adequate for daily office documents.
...
1.  **Enable Windows Data Deduplication:** Set up a scheduled data deduplication job via the FSx console or PowerShell. This is a zero-risk operation that typically yields **50–60% storage savings** on user directories and file shares.
2.  **Right-Size Throughput Dynamically:** Check CloudWatch metrics for `ThroughputLimit` and `ThroughputUtilization`. 
    *   Scale down the provisioned throughput capacity during normal operating hours to match your peak utilization.
    *   You can dynamically increase throughput (online, with zero downtime) before executing large migrations, and scale it back down immediately after completion.
3.  **Migrate Dev/Test to Single-AZ:** Audit all running FSx for Windows file systems. Re-create any non-production file systems as **Single-AZ** to achieve immediate **~50% savings** on storage, throughput, and IOPS.
4.  **Use HDD Storage Tiers for User File Shares:** For general-purpose user directories, backups, and office document shares, use **HDD storage**. Combined with the SSD cache (automatically managed by FSx), this provides excellent read/write speeds for active files while keeping the bulk capacity at the cheap **$0.013/GB-month** tier.
