...
4.  **Intelligent-Tiering:** Elastic billing model. Automatically tiers data across Frequent Access, Infrequent Access, and Archive Instant Access tiers based on access patterns, starting under **$0.005 per GB-month**.
5.  **Backup Storage:** Charged per GB-month for data backed up to AWS Backup.
6.  **Metadata Storage:** Some Persistent configurations charge separately for metadata storage.
...
*   **Intelligent-Tiering Storage:** Fully elastic. Automatically transitions cold data to cheaper Infrequent and Archive tiers. Archive storage starts as low as **$0.004 per GB-month**. No capacity provisioning is required; you are billed for exactly what you store.
...
*   *The Multiplier Effect:* A 10 TB file system on the 1000 MB/s/TB tier costs **$8,300.00/month** compared to **$1,330.00/month** on the 12 MB/s/TB tier.
...
### Example Monthly Cost Calculation (Throughput Impact)
*Workload: A Persistent Single-AZ SSD storage pool has 5 TB of capacity. We compare the monthly costs of the 40 MB/s/TB tier vs the 500 MB/s/TB tier.*

*   **Option A: 40 MB/s/TB Throughput Tier:**
    $$\text{Monthly Cost} = 5,000\text{ GB} \times \$0.17/\text{GB} = \$850.00\text{ / month}$$
*   **Option B: 500 MB/s/TB Throughput Tier:**
    $$\text{Monthly Cost} = 5,000\text{ GB} \times \$0.49/\text{GB} = \$2,450.00\text{ / month}$$
*   **Cost Variance:** **$1,600.00/month** (A **188% price hike** for performance scaling).
...
*   **Over-Provisioning Storage for Throughput:** Provisioning larger storage sizes just to hit a specific throughput target, paying for unused storage space.
*   **Leaving Persistent Multi-AZ Running for Temp Jobs:** Launching Multi-AZ Persistent file systems for temporary machine learning runs or rendering jobs, doubling the compute and replication baseline cost.
*   **Forgetting to Delete Scratch Volumes:** Leaving "temporary" Scratch file systems running active billing indefinitely until manually deleted.
...
1.  **Use Scratch Tiers for Ephemeral Compute:**
    *   For all ML training jobs and batch rendering, deploy **Scratch SSD** file systems. Hook them up to S3 to auto-load training data, and write results back to S3. Delete the Scratch file system immediately after the job finishes.
    *   **The Savings:** Slashes storage compute rates to **$0.043/GB-month**.
2.  **Enable LZ4 Data Compression:**
    *   Enable **LZ4 compression** on the Lustre file system to compress files automatically.
    *   **The Savings:** Reduces the total GB-month storage footprint by **30–50%** for typical text or log datasets.
3.  **Evaluate Intelligent-Tiering for Mixed Datasets:**
    *   For workloads with highly variable access patterns (hot/cold mixed data), use the **Intelligent-Tiering** storage class to automatically slide data to the archive tier ($0.004/GB-month) without upfront capacity allocation.
4.  **Use HDD Tiers with SSD Cache for Large Archives:**
    *   For large datasets read sequentially, use the **Persistent HDD** storage class. The built-in 20% SSD cache automatically caches hot metadata for SSD speed, while base storage bills at the cheap **$0.013/GB-month** rate.
5.  **Utilize S3 Prefix Filters during Sync:**
    *   Restrict the data repository association to specific prefixes (folders) containing only the active training dataset. This reduces metadata synchronization time and S3 API request fees.
