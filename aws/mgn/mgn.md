...
## 2. Billing Mechanics
1. **Replication Free Period:** **90 days (2,160 hours) FREE per source server** registered in MGN (begins automatically when the replication agent connects).
...
3. **Underlying Staging Infrastructure:** Billed for standard AWS resources provisioned during continuous replication:
   ...
   * *Data Transfer:* Inbound data transfer to AWS is free; standard egress applies during testing.
...
* **Free Window:** 90 days free per source server (2,160 hours).
* **Post 90-Day Rate:** $0.042 per server per hour ($1.008 per server per day / $30.24 per month).

---

## 5. AWS Free Tier Coverage
* **AWS MGN Free Tier:** Includes **90 days of free replication** for each server added to AWS MGN.
...
* **Replication Stagnation (Exceeding the 90-Day Free Window):**
  ...
  * *Math:* Replicating 200 idle servers past 90 days costs:
    $$200\text{ servers} \times \$30.24\text{ / month} = \mathbf{\$6,048.00\text{ / month in server fees!}}$$
    This is in addition to staging EBS volume and EC2 compute charges.
...
3. **Delete Disconnected Source Servers Post-Cutover:** Once production cutover is verified, disconnect and remove source servers from the MGN console to terminate staging EBS volumes and EC2 replication servers immediately.
