...
## 1. Service Overview
AWS Elemental MediaConvert is a file-based video transcoding service that transforms video content (VOD archives, user-generated video uploads, movies) into adaptive bitrate (ABR) formats (HLS, DASH) or MP4 files for high-quality playback on smartphones, tablets, PCs, and smart TVs. MediaConvert handles complex video processing tasks including frame rate conversion, graphic overlays, multi-channel audio, captioning, and DRM encryption. MediaConvert is billed per minute of output video created using a standardized **"Normalized Minutes"** metric.

---

## 2. Billing Mechanics
1. **Output Video Duration & Normalized Minutes:** Billed per minute of output video generated (in 1-second increments). Rates use a **Normalized Minutes** multiplier based on:
   * *Resolution:* SD (up to 480p), HD (up to 1080p), 4K/UHD (above 1080p).
   * *Codec:* AVC (H.264), HEVC (H.265), AV1, Apple ProRes.
   * *Frame Rate:* Standard (<= 30 fps) vs High Frame Rate (> 30 fps).
2. **Service Tiers:**
   * *Basic Tier:* Simple single-file MP4 outputs ($0.0075 per minute for HD).
   * *Professional Tier:* Advanced ABR packaging, multi-language audio, DRM, graphic overlays ($0.0150 per minute for HD AVC).
3. **Volume Tier Discounts:** Automatically discounts rates as monthly "normalized minutes" increase across volume thresholds.

---

## 3. Key Cost Dimensions

### A. Professional Tier Rates (us-east-1 AVC Codec Standard Frame Rate)
...
### B. Basic Tier Rates (us-east-1 AVC Codec)
...
| **HD Output (1080p)** | Professional Tier | **$0.0150** | **$900.00** |
| **4K Output (UHD)** | Professional Tier | **$0.0300** | **$1,800.00** |
...
* **Generating Unnecessary Fixed High-Bitrate ABR Renditions:** Encoding an 8-profile ABR stack for simple low-motion talk shows or lectures, generating multiple redundant 1080p/4K renditions.
...
3. **Use Accelerated Transcoding for High-Priority VOD:** Enable Accelerated Transcoding for time-sensitive jobs (compresses processing time up to 25x with minimal cost impact).
