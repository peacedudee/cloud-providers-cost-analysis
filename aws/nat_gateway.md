...
## 1. Service Overview
AWS NAT Gateway is a fully managed Network Address Translation service that enables instances in a private subnet to connect to the internet or other AWS services, while preventing the internet from initiating connections with those instances. NAT Gateway is widely recognized as one of the most common source of high networking bills in AWS due to its dual billing structure (hourly base fee + per-GB data processing fee) and the fact that it is often deployed redundantly across multiple Availability Zones.

---

## 2. Billing Mechanics
NAT Gateway billing is calculated on a monthly cycle based on the following dimensions:
1.  **NAT Gateway Hourly Base Fee:** A flat fee charged per hour or partial hour that the gateway is active.
2.  **Data Processing Fee:** Billed per GB of data processed (sent or received) through the gateway.
3.  **Public IPv4 / Elastic IP Surcharge:** Billed per hour for the public Elastic IP required for the NAT Gateway to function.
4.  **Network Egress Fees:** Billed standard AWS Internet Data Transfer Out rates for outbound traffic.
