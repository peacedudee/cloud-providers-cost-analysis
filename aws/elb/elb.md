...
*   *The Billing Rule:* Capacity units are calculated by evaluating four different dimensions (new connections, active connections, processed bytes, and rule evaluations). You are billed **only for the single dimension with the highest usage** in any given hour.
...
### Example Capacity Unit Calculation (Rule Evaluation Impact)
*Workload: A production Application Load Balancer has 80 path-based routing rules configured. It handles a steady traffic rate of 5,000 requests per second. The rule evaluation count is:*

*   **Rule Evaluations per Second:**
    $$\text{Evaluations/sec} = 5,000\text{ requests/sec} \times 80\text{ rules} = 400,000\text{ evals/sec}$$
*   **Required LCUs (1 LCU processes 1,000 rules/sec):**
    $$\text{LCUs Required} = \frac{400,000}{1,000} = 400\text{ LCUs}$$
*   **Hourly Capacity Fee:**
    $$\text{Capacity Fee} = 400\text{ LCUs} \times \$0.008 = \$3.20\text{ / hour}$$
*   **Monthly Capacity Surcharge:**
    $$\text{Monthly Fee} = \$3.20/\text{hour} \times 730\text{ hours} = \$2,336.00\text{ / month}$$
*   This represents a massive billing hotspot driven entirely by high rule counts and request rates.
...
*   **Rule Evaluation Storms (ALB):** Creating complex path-based or header-based routing rules on an ALB that processes millions of requests.
...
2.  **Delete Idle/Orphaned Load Balancers:**
    *   Set up AWS Config rules or scripts to identify load balancers with `ActiveConnectionCount = 0` over the last 14 days and delete them.
3.  **Consolidate Applications onto Shared ALBs:**
    *   Instead of deploying a separate ALB for every microservice, use a single shared ALB with **Host-Based Routing** (using wildcards or subdomains) or **Path-Based Routing** to consolidate up to 100 applications under one base hourly charge.
4.  **Use NLB for Non-HTTP TCP Traffic:**
    *   If your application only needs simple TCP port forwarding (without TLS termination or HTTP header inspection), deploy an **NLB**. NLB baseline hourly fees and capacity units ($0.006/NPU vs. $0.008/LCU) are cheaper.
5.  **Use API Gateway HTTP APIs for Lambda Routing:**
    *   If you are routing traffic to AWS Lambda, use **API Gateway HTTP APIs** instead of an ALB. This avoids both the flat ALB hourly base fee and the 2.5x Lambda target LCU processed bytes penalty.
