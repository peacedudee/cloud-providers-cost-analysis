...
1. **Active Greengrass Core Device Monthly Fee:** Billed monthly per active core gateway device that connects to the Greengrass cloud control plane ($0.16 per active core device per month).
   * A core device is considered "active" and billed for a month if it connects or authenticates with the AWS Cloud at least once during that calendar month. If it runs entirely offline for a month, it is not billed for that month.
   * **Large Fleets:** Volume discounts or custom pricing are available for fleets exceeding 10,000 active devices.
2. **Cloud Messaging & Data Storage:** Local messaging between edge devices connected to a Greengrass core is 100% free ($0.00). Standard IoT Core messaging rates apply only when data is synced to the AWS cloud.
...
* **Local Container & ML Execution:** $0.00 (processed locally on the device's CPU/GPU).

---

## 5. AWS Free Tier Coverage
* **AWS IoT Greengrass Free Tier:** Includes the first **3 active core devices per month for free** for the first 12 months for new accounts.
...
* **Failing to Properly Decommission Offline Core Devices:**
  * If a gateway device is retired from the physical field but still occasionally turns on and authenticates with AWS, it will continue to incur the monthly $0.16 charge.
  * **Rule to Avoid:** Perform a clean uninstall procedure on the device (stop the Greengrass nucleus) and delete the core device definition from the AWS IoT Greengrass console.
...
2. **Leverage Free Local Inter-Device Communication:**
   * Configure local client devices (sensors/cameras) to publish MQTT messages directly to the Greengrass Core locally for $0.00 messaging charges.
3. **Audit Active Device Gateways Monthly:**
   * Use the AWS billing console or scripts to audit the active Greengrass devices. Uninstall and delete any retired devices to avoid phantom $0.16/device monthly costs.
