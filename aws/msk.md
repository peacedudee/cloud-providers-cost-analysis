...
1.  **MSK Provisioned Clusters:**
    *   *Broker Node Hours:* Billed hourly per broker instance based on instance family and size (e.g. `kafka.m5.large`).
...
2.  **MSK Serverless Clusters:**
    *   *Cluster Hours:* A flat hourly charge per provisioned serverless cluster.
    *   *Partition Hours:* Billed hourly for active partition counts.
    *   *Data Ingestion / Retrieval:* Billed per GB written and read.
    *   *Storage:* Billed per GB-month of message storage.
...
*   **Broker Instance Hours:** You pay per-second (with a 1-minute minimum) for each running broker.
    ...
    *   *kafka.m5.large (Baseline Production Node):* **$0.21 per hour** (~$153.30/month).
...
*   **Cluster Base Fee:** **$0.75 per cluster-hour** (in `us-east-1`), creating a flat idle base fee of:
    $$\$0.75 \times 730\text{ hours} = \$547.50\text{ / month flat}$$
...
| **Standard Storage**| **$0.1000 / GB-month** | **$0.1000 / GB-month** | EBS volume / S3 retention |
...
*   **High Cross-AZ Replication Fees:** Ingesting large volumes of uncompressed data into Kafka, driving up inter-broker cross-AZ replication transfer charges.
...
1.  **Break-Even Sizing (Serverless vs. Provisioned):**
    ...
    *   Use **Provisioned Mode with `kafka.m5` nodes** for large, steady-state production workloads processing **more than 3 TB of data per month**.
2.  **Tiered Storage (Offload EBS to S3):**
    *   Enable **Tiered Storage** on your Provisioned MSK clusters.
    *   Configure a short retention window (e.g., 2 hours or 12 hours) on the local high-cost broker EBS storage (**$0.10/GB**).
    *   Let MSK automatically push older message data to **Amazon S3 Tiered Storage** (**$0.023/GB-month**), cutting storage costs by **77%**.
