...
1.  **Key Storage Fees:** Billed hourly per provisioned Customer Managed Key (CMK).
2.  **API Requests:** Billed per 10,000 API operations (e.g., `Encrypt`, `Decrypt`, `GenerateDataKey`).
...
*   **Customer Managed Keys (CMK):**
    ...
    *   *Key Rotation:* Enabling automatic key rotation preserves previous key versions. You pay **$1.00 per month** for each rotated version kept.
...
*   **Symmetric Key Operations (`GenerateDataKey`, `Decrypt`, `Encrypt`, `ReEncrypt`):**
    *   *Rate:* **$0.03 per 10,000 requests** ($0.000003 per request).
...
*   If you configure KMS to use a Custom Key Store backed by AWS CloudHSM:
    ...
    *   *Minimum:* A production CloudHSM cluster requires at least 2 HSMs in multi-AZ ($2,117.00/month base).
